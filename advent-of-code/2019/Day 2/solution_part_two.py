...
# https://riptutorial.com/python/example/2920/argument-passing-and-mutability
def process_intcode(input):
  opcodes = input.copy()
  for address in range(len(opcodes)):
    if ((address % 4) != 0): continue
    instruction = opcodes[address]

    parameterA_address = opcodes[address + 1]
    parameterB_address = opcodes[address + 2]
    parameterC_address = opcodes[address + 3]
...
for noun in range(100):
  for verb in range(100):
    ...
    result = process_intcode(input)
    # https://www.programiz.com/python-programming/string-interpolation
    if (result[0] == 19690720):
      print('100 * %d + %d = %d' %(noun, verb, 100 * noun + verb))